...
float3 GetColorForRay(in float3 startRayPos, in float3 startRayDir, inout uint rngState)
{
    ...
    for (int bounceIndex = 0; bounceIndex <= /*$(Variable:numBounces)*/; ++bounceIndex)
    {
        ...
        if (hitInfo.dist == c_superFar)
        {
			...
			{
				case Skybox::Arches_E_PineTree_3k: texel = /*$(Image:Arches_E_PineTree_3k.hdr:RGBA32_Float:float4:false)*/.SampleLevel(texSampler, uv, 0).rgb; break;
				case Skybox::satara_night_4k_hdr: texel = /*$(Image:satara_night_4k.hdr:RGBA32_Float:float4:false)*/.SampleLevel(texSampler, uv, 0).rgb; break;
                case Skybox::satara_night_4k_exr: texel = /*$(Image:satara_night_4k.exr:RGBA32_Float:float4:false)*/.SampleLevel(texSampler, uv, 0).rgb; break;
			}
