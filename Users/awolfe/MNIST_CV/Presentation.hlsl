...
/*$(_compute:Presentation)*/(uint3 DTid : SV_DispatchThreadID)
{
    ...
    // Draw the hidden layer activations
    {
        ...
        if (relPos.x >= 0 && relPos.y >= 0 && relPos.x < c_hiddenPanelSize.x && relPos.y < c_hiddenPanelSize.y)
        {
            ...
            relPos.y /= 31;
            float value = bool(/*$(Variable:UseCooperativeVectors)*/) ? CVHiddenLayerActivations[relPos.y] : HiddenLayerActivations[relPos.y];
            PresentationCanvas[DTid.xy] = float4(value.xxx, 1.0f);
            return;
        }
    ...
    // Draw the output layer activations
    {
        ...
        if (relPos.x >= 0 && relPos.y >= 0 && relPos.x < c_outputPanelSize.x && relPos.y < c_outputPanelSize.y)
        {
            ...
            relPos.y /= 31;
            float value = bool(/*$(Variable:UseCooperativeVectors)*/) ? CVOutputLayerActivations[relPos.y] : OutputLayerActivations[relPos.y];
            PresentationCanvas[DTid.xy] = float4(value.xxx, 1.0f);
            return;
        }
    ...
    // Draw the output layer labels
    {
        ...
        if (relPos.x >= 0 && relPos.y >= 0 && relPos.x < c_outputLabelsSize.x && relPos.y < c_outputLabelsSize.y)
        {
            if ((relPos.y % 31) < 28)
            {
                ...
                if (alpha > 0.0f)
                {
                    float outputActivation = bool(/*$(Variable:UseCooperativeVectors)*/) ? CVOutputLayerActivations[index] : OutputLayerActivations[index];
                    float3 pixelColor = lerp(float3(0.4f, 0.0f, 0.0f), float3(1.0f, 1.0f, 0.0f), outputActivation);
