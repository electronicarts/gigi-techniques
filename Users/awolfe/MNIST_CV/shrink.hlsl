...
void ShrinkFnNormalize(uint2 pixelPos)
{
    ...
    uint index = pixelPos.y * 28 + pixelPos.x;
    CVNNInput[index] = output;
}
...
/*$(_compute:Shrink)*/(uint3 DTid : SV_DispatchThreadID)
{
    if (/*$(Variable:UseImportedImage)*/)
    {
        ...
        uint index = DTid.y * 28 + DTid.x;
        CVNNInput[index] = ImportedImage[DTid.xy];
    }
    else if (/*$(Variable:NormalizeDrawing)*/)
        ShrinkFnNormalize(DTid.xy);
    else
        ShrinkFn(DTid.xy);
}
